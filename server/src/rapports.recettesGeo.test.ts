...
test('GET /api/rapports/recettes-geographiques ne charge pas z.geometry pour les rapports recouvrement non geographiques', async () => {
  await withTestContext(async (ctx) => {
    const fx = resetFixtures(ctx);
    const originalPrepare = ctx.db.prepare.bind(ctx.db);
    const seenQueries: string[] = [];

    // @ts-ignore capture SQL generated by the report route
    ctx.db.prepare = ((sql: string) => {
    ...
    try {
      const res = await requestGeoReport(ctx, {
        path: '/api/rapports/recouvrement?annee=2026&ventilation=zone',
        headers: makeAuthHeader(ctx, fx.financier),
      });

      assert.equal(res.status, 200);
      const recouvrementQuery = seenQueries.find((sql) => sql.includes('FROM titres t') && sql.includes('JOIN lignes_declaration ld ON ld.declaration_id = dec.id'));
      assert.ok(recouvrementQuery);
      assert.equal(recouvrementQuery?.includes('z.geometry AS zone_geometry'), false);
    } finally {
      // @ts-ignore restore monkey patch after SQL capture
      ctx.db.prepare = originalPrepare;
    }
  });
});

test('GET /api/rapports/recettes-geographiques reutilise la geometrie deja normalisee pour les lignes suivantes d une meme zone', async () => {
  await withTestContext(async (ctx) => {
    const fx = resetFixtures(ctx);
    const originalPrepare = ctx.db.prepare.bind(ctx.db);

    // @ts-ignore inject a malformed duplicate geometry on the second row of the same zone
    ctx.db.prepare = ((sql: string) => {
      const statement = originalPrepare(sql);
      if (!sql.includes('z.geometry AS zone_geometry') || !sql.includes('FROM titres t')) {
        return statement;
      }
      ...
      return {
        ...
        all: (...args: unknown[]) => {
          ...
          let duplicatedZoneSeen = false;
          return rows.map((row) => {
            if (row.zone_code === 'ZC') {
              if (duplicatedZoneSeen) {
                return { ...row, zone_geometry: '{"type":"Point","coordinates":[2,48]}' };
              }
              duplicatedZoneSeen = true;
            }
            return row;
          });
        },
    ...
    try {
      ...
      assert.equal(res.status, 200);
      assert.equal(res.json?.zones.length, 2);
      assert.equal(res.json?.zones.find((zone: { zone_code: string }) => zone.zone_code === 'ZC')?.montant_recouvre, 550);
    } finally {
      // @ts-ignore restore monkey patch after fault injection
      ctx.db.prepare = originalPrepare;
    }
